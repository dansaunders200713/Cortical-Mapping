function [string, fullstring] = GetEchoString3(window_pointer, msg, x, y, textColor, bgColor, sequence)
...
jumbled_keyboard_map = [102 110 104 105 100 115 121 103 108 112 98 111 117 116 118 99 122 107 101 119 97 120 113 114 106 109];
...
while true
    ...
    switch abs(char)
        ...
        otherwise
            if (isletter(char))
                string = [string, jumbled_keyboard_map(char-96)]; %#ok<AGROW>
                fullstring = [fullstring, jumbled_keyboard_map(char-96)];
            else
                string = [string, char];
