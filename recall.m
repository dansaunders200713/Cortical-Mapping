...
for n = 1:75
    ...
    if etime(clock, start_time) > 900 % if timer exceeds 15 minutes
        break % the block has completed
    end
end

% end jumbled keyboard portion
...
recall_filename = ['recall' subject_code '.txt'];

% write the info to file
fid = fopen(recall_filename, 'w');
